...
MX implements a number of standard interactive analysess: (i) data reordering, sorting, and searching; (ii) data visualization via heatmaps , scatter/box/violin plots, marginal distributions, and 2D embedding; (iii) data exploration via k-means clustering, dendrogram, correlation and covariance matrix computation, and outlier detection. We validated MX by showing that it produces reasonable results for the iris flower dataset. We then used Matrix Explorer to analyze a novel SPECT image dataset, revealing sex differences that were previously unknown.
\end{abstract}
